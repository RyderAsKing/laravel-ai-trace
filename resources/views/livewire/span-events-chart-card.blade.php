<x-ai-trace::card :cols="$this->cols" wire:poll.10s="">
    <x-ai-trace::card-header name="Span Events" :details="'Past '.$periodLabel" />

    <p class="text-3xl font-semibold tabular-nums text-gray-800 sm:text-4xl dark:text-gray-100">{{ number_format($series['total']) }}</p>
    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">span events recorded</p>
    ...
    <x-ai-trace::multi-line-chart
        event-name="ai-trace-span-events-chart-update"
        :series="$series"
